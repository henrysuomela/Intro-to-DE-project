...
def transform_data(**kwargs):
    ...
    # Make sure formatted_date is datetime object
    daily_avg_df['formatted_date'] = pd.to_datetime(daily_avg_df['formatted_date'])
...
def validate_data(**kwargs):
    ...
    # 1. Missing Values Check (Daily and Monthly)
    if daily_avg_df.isnull().any().any():
        raise ValueError("Validation failed: Missing values detected in daily data.")
    if monthly_avg_df.isnull().any().any():
        raise ValueError("Validation failed: Missing values detected in monthly data.")
    
    # 2. Range Check
    if not ((daily_avg_df['temperature_c'].between(-50, 50)).all() and
            (daily_avg_df['humidity'].between(0, 1)).all() and
            (daily_avg_df['wind_speed_kmh'] >= 0).all()):
        raise ValueError("Validation failed: Daily values out of expected ranges detected.")

    if not ((monthly_avg_df['avg_temperature_c'].between(-50, 50)).all() and
            (monthly_avg_df['avg_humidity'].between(0, 1)).all() and
            (monthly_avg_df['avg_wind_speed_kmh'] >= 0).all() and
            (monthly_avg_df['avg_visibility_km'] >= 0).all() and
            (monthly_avg_df['avg_pressure_millibars'].between(850, 1080)).all()):
        raise ValueError("Validation failed: Monthly values out of expected ranges detected.")
    

    """ Outlier Detection, flags negative summer temperatures and over 20C temperatures in the winter """
    # Daily
    # Create new column for month
    daily_avg_df['formatted_date'] = pd.to_datetime(daily_avg_df['formatted_date'])
    daily_avg_df['month'] = daily_avg_df['formatted_date'].dt.month

    # Filter for flagged values
    flagged_values_daily = daily_avg_df[
        ((daily_avg_df['month'].isin([5, 6, 7])) & (daily_avg_df['temperature_c'] < 0)) |  # Negative in summer
        ((daily_avg_df['month'].isin([11, 12, 1, 2])) & (daily_avg_df['temperature_c'] >= 20))  # 20+ in winter
    ]

    # Print flagged values if found
    if not flagged_values_daily.empty:
        print("Extreme outliers found in the daily table:")
        print(flagged_values_daily)

    # Drop the month column
    daily_avg_df = daily_avg_df.drop(columns=['month'])


    # Monthly
    # Filter for flagged values
    flagged_values_monthly = monthly_avg_df[
        ((monthly_avg_df['month'].isin([5, 6, 7])) & (monthly_avg_df['avg_temperature_c'] < 0)) |  # Negative in summer
        ((monthly_avg_df['month'].isin([11, 12, 1, 2])) & (monthly_avg_df['avg_temperature_c'] >= 20))  # 20+ in winter
    ]
...
def load_data(**kwargs):
    ...
    daily_avg_df.to_sql('daily_weather', conn, if_exists='append', index=False)
    monthly_avg_df.to_sql('monthly_weather', conn, if_exists='append', index=False)
