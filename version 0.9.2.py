...
def validate_data(**kwargs):
    ...
    if daily_avg_df.isnull().sum().sum() > 0:
        raise ValueError("Missing values found in daily_avg_df!")
    if monthly_avg_df.isnull().sum().sum() > 0:
        raise ValueError("Missing values found in monthly_avg_df!")
    
    # Range Check
    if not daily_avg_df['temperature_c'].between(-50, 50).all():
        raise ValueError("Temperature values are out of range in daily_avg_df!")
    if not daily_avg_df['humidity'].between(0, 1).all():
        raise ValueError("Humidity values are out of range in daily_avg_df!")
    if not daily_avg_df['wind_speed_kmh'].ge(0).all():
        raise ValueError("Wind speed values are out of range in daily_avg_df!")
    
    '''if not monthly_avg_df['temperature_c'].between(-50, 50).all():
        raise ValueError("Temperature values are out of range in daily_avg_df!")
    if not monthly_avg_df['humidity'].between(0, 1).all():
        raise ValueError("Humidity values are out of range in daily_avg_df!")
    if not monthly_avg_df['wind_speed_kmh'].ge(0).all():
        raise ValueError("Wind speed values are out of range in daily_avg_df!")'''
    
    # Outlier Detection
    outliers = daily_avg_df[(daily_avg_df['temperature_c'] < -50) | (daily_avg_df['temperature_c'] > 50)]
    if not outliers.empty:
        print(f"Warning: Found temperature outliers in daily data:\n{outliers}")
    
    '''outliers = monthly_avg_df[(daily_avg_df['temperature_c'] < -50) | (monthly_avg_df['temperature_c'] > 50)]
    if not outliers.empty:
        print(f"Warning: Found temperature outliers in monthly data:\n{outliers}")'''
